...
支持云盘原生分享/转存的数据源（百度网盘、夸克网盘，以及配了[官网令牌](#官网令牌推荐免扫码--分享转存)的阿里云盘）在文件列表选中条目后可「分享」，也能「导入分享」把别人的分享转存进当前目录。导出时可选两种格式：
...
导入分享自动按链接识别格式：`sd://` 开头走标准协议（自带密码）；否则按域名识别是哪家网盘的官网分享短链（百度 `pan.baidu.com`、阿里 `alipan.com`/`aliyundrive.com`、夸克 `pan.quark.cn`）。**默认不必单独填提取码** —— 直接粘贴带 `?pwd=`/`?passcode=` 的链接即可自动读取；没带密码时先探测是否免密分享，确实需要提取码才弹框补填后重试。识别不出的链接会明确报错。
