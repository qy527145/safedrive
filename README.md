...
2. **数据源管理** —— 添加百度网盘（BDUSS 自动换取开放平台令牌）、WebDAV 或本地文件系统数据源，绑定策略
...
最简配置只需填写登录百度网盘账号的 `BDUSS` 值。首次连接时，服务会参考 onepan 的流程申请 OAuth 设备码、使用 BDUSS 完成设备授权并自动换取 Access/Refresh Token；令牌过期后会自动刷新，并将百度返回的新令牌原子写回 `datasources.json`。
...
- 百度网盘 BDUSS、可选 Client Secret 与自动换取的 Access/Refresh Token 明文保存在 `datasources.json`；必须像根密码一样保护 `--data-dir`，凭证失效后需在数据源管理中更新
