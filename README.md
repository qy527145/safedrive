...
官网令牌可留空 —— 创建分享改由开放平台接口完成（无需官网令牌）；但**导入转存**、分享/转存里需要降级到官网接口的情形，以及备份盘文件的**快传**降级，都依赖官网令牌，留空则这些不可用，其余功能照常。两条令牌线各自独立轮换、互不影响，编辑数据源时也不会把对方回退成表单里的旧值。
...
支持云盘原生分享/转存的数据源（百度网盘、夸克网盘、阿里云盘）在文件列表选中条目后可「分享」，也能「导入分享」把别人的分享转存进当前目录。其中阿里云盘创建分享优先走[开放平台](https://www.alipan.com/developer)接口（无需[官网令牌](#官网令牌推荐免扫码--分享转存)），失败再降级到官网接口乃至快传；导入转存仍依赖官网令牌。导出时可选两种格式：
