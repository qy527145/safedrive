...
interface TasksState {
  tasks: TransferTask[];
  enqueue: (task: Omit<TransferTask, 'status' | 'doneBytes'>, run: Runner) => void;
...
export const useTasks = create<TasksState>()((set, getState) => ({
  ...
  enqueue: (task, run) => {
    ...
    set((s) => ({ tasks: [...s.tasks, { ...task, status: 'queued', doneBytes: 0 }] }));
    pump(set, getState);
  },
  ...
  void run({ ...next }, ctrl.signal)
    .then(() => {
      ...
      if (cur?.status === 'running') {
        _patch(next.id, { status: 'done', doneBytes: cur.totalBytes });
      }
    })
    .catch((e: unknown) => {
      const cur = getState().tasks.find((t) => t.id === next.id);
