 ...
 * 建策略 → 建数据源 → 上传 → 云端形态校验 → 刷新 → /stream 预览 →
 * 原生下载字节比对 → 重命名 → 删除。
...
test('完整 UI 流程：策略 → 数据源 → 上传 → 预览/下载 → 重命名 → 删除', async ({ page }) => {
  // ---- 无密码模式直接进入主界面 ----
  await page.goto('/');
  ...
  await page.getByLabel('策略名称').fill('默认策略');
  await expect(page.getByLabel('上传分卷大小')).toBeVisible();
  await expect(page.getByLabel('根密码')).toBeVisible();
  await page.getByRole('button', { name: /确\s*定/ }).click();
  await expect(page.getByRole('cell', { name: '默认策略' })).toBeVisible();

  // ---- 创建数据源 ----
  await page.getByText('数据源管理').click();
  await page.getByRole('button', { name: '添加数据源' }).click();
  await page.getByLabel('数据源名称').fill('本地测试');
  await page.getByLabel(/根目录/).fill(storageRoot);
  await page.getByRole('button', { name: /确\s*定/ }).click();
  await expect(page.getByRole('cell', { name: '本地测试' })).toBeVisible({ timeout: 15_000 });

  // ---- 进入浏览器并上传 ----
  await page.getByText('数据管理').click();
  await page.locator('.ant-card').filter({ hasText: '本地测试' }).click();
  ...
  expect(rootEntries[0]).toMatch(/^[\u4e00-\u8e06]+$/);
